...
import { ListPenalties } from "./list-penalties";
import { ListCadreCandidates } from "./list-cadre-candidates";

export const DaoPortalFeatures = () => {
  const {
    selectedAccount,
    isUserAuthenticated,
    authenticateUser,
  ...
  if (isUserCadre && isUserAuthenticated) return <DaoPortalView />;
};

const DaoPortalView = () => {
  return (
    <Tabs defaultValue="dao-candidates" className="w-full">
      <TabsList className="grid w-full grid-cols-2">
        <TabsTrigger value="dao-candidates">Curator DAO Candidates</TabsTrigger>
        <TabsTrigger value="penalties">Penalties</TabsTrigger>
      </TabsList>
      <TabsContent value="dao-candidates">
        <ListCadreCandidates />
      </TabsContent>
