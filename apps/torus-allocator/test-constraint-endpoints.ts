...
async function testConstraintEndpointsInAllocator() {
  ...
  try {
    ...
    const permissionConstraint = ConstraintBuilder.create(
      ...
      BoolExpr.base(BaseConstraint.permissionExists('0x1ef9a7882d5045650e8cda34cbd9132ca8c69533702da7582e9925cc6f6279fd'))
    );
    ...
    try {
      ...
      console.log('   - First 200 chars:', networkState.visualization.substring(0, 200) + '...');
    } catch (stateError) {
      console.log('❌ Network State Failed:', stateError instanceof Error ? stateError.message : String(stateError));
    }
    console.log();
    ...
    try {
      ...
      console.log('⚠️ Permission Constraint Error:', 
    ...
    try {
      ...
      console.log('   - Visualization Length:', finalState.visualization.length);
      
      // Count different sections
      const hasAlphaNodes = finalState.visualization.includes('ALPHA NODES');
      const hasProductionNodes = finalState.visualization.includes('PRODUCTION NODES');
    ...
    console.log('📝 Note: Make sure torus-allocator app is running on port 3023');
