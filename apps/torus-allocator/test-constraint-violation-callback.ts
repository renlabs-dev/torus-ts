...
async function testConstraintViolationCallback() {
  ...
  try {
    ...
    try {
      ...
      console.log('❌ Health Check Failed:', healthError instanceof Error ? healthError.message : String(healthError));
    ...
    console.log('📝 This should activate and NOT trigger the violation callback');
    ...
    const alwaysPassConstraint = ConstraintBuilder.create(
      ...
      BoolExpr.comp(CompOp.Gte, NumExpr.stakeOf(TEST_USER_ADDRESS), NumExpr.literal(0)) // stakeOf >= 0 (always true)
    );
    ...
    try {
      ...
      console.log('   - Fetched Facts Count:', alwaysPassResult.fetchedFactsCount);
      
      // Wait a moment for any potential violations to be processed
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      if (alwaysPassResult.activated) {
        console.log('✅ Expected: Constraint activated (stake >= 0 should always be true)');
      } else {
        console.log('⚠️  Unexpected: Constraint not activated - might indicate an issue');
      }
      
    } catch (alwaysPassError) {
      console.log('❌ Always-Pass Constraint Error:', 
        alwaysPassError instanceof Error ? alwaysPassError.message : String(alwaysPassError));
    }
    console.log();
    ...
    console.log('📝 This should NOT activate and should trigger the violation callback');
    ...
    const alwaysFailConstraint = ConstraintBuilder.create(
      ...
      BoolExpr.comp(CompOp.Lt, NumExpr.stakeOf(TEST_USER_ADDRESS), NumExpr.literal(0)) // stakeOf < 0 (always false)
    );
    ...
    try {
      ...
      console.log('   - Fetched Facts Count:', alwaysFailResult.fetchedFactsCount);
      ...
      if (!alwaysFailResult.activated) {
        console.log('✅ Expected: Constraint not activated (stake < 0 should always be false)');
        console.log('🚨 Check server console for violation callback messages!');
      } else {
        console.log('⚠️  Unexpected: Constraint activated - this should not happen for stake < 0');
      }
      
    } catch (alwaysFailError) {
      console.log('❌ Always-Fail Constraint Error:', 
        alwaysFailError instanceof Error ? alwaysFailError.message : String(alwaysFailError));
    }
    console.log();
    ...
    if (alwaysPassResult?.productionId) {
      ...
      try {
        const passEvaluation = await client.constraint.getEvaluationStatus.query({
        ...
        if (passDetailed.status === 'satisfied') {
          ...
          console.log('⚠️  Status:', passDetailed.status, '- Expected satisfied for stake >= 0');
        }
    ...
    if (alwaysFailResult?.productionId) {
      ...
      try {
        const failEvaluation = await client.constraint.getEvaluationStatus.query({
        ...
        if (failDetailed.status === 'violated') {
          ...
          console.log('⚠️  Status:', failDetailed.status, '- Expected violated for stake < 0');
        }
    ...
    try {
      const nonExistentStatus = await client.constraint.getEvaluationStatus.query({
    ...
    console.log('🔍 Testing OR Expression: (stake < 0) OR permissionExists(0x2111...)');
    const orConstraint = ConstraintBuilder.create(
      '0x1ef9a7882d5045650e8cda34cbd9132ca8c69533702da7582e9925cc6f6279fd',
      BoolExpr.or(
        BoolExpr.comp(CompOp.Lt, NumExpr.stakeOf(TEST_USER_ADDRESS), NumExpr.literal(0)), // False
        BoolExpr.base(BaseConstraint.permissionExists('0x211142bf6d4d1d08afec9bd2f8be8c27f318b0524992b22849734a2375f74df1'))
      )
    );
    
    try {
      const orResult = await client.constraint.addTest.mutate({ constraint: orConstraint });
      ...
      console.log('   Expected: satisfied if permission 0x2111... exists, violated if not');
    } catch (error) {
    ...
    const notConstraint = ConstraintBuilder.create(
      ...
      BoolExpr.not(
        BoolExpr.comp(CompOp.Lt, NumExpr.stakeOf(TEST_USER_ADDRESS), NumExpr.literal(0)) // False -> NOT makes it True
      )
    );
    
    try {
      const notResult = await client.constraint.addTest.mutate({ constraint: notConstraint });
    ...
    console.log('🔍 Testing Nested Expression: permissionExists(0x2111...) AND (stake >= 0)');
    const nestedConstraint = ConstraintBuilder.create(
      '0x1ef9a7882d5045650e8cda34cbd9132ca8c69533702da7582e9925cc6f6279fd',
      BoolExpr.and(
        BoolExpr.base(BaseConstraint.permissionExists('0x211142bf6d4d1d08afec9bd2f8be8c27f318b0524992b22849734a2375f74df1')),
        BoolExpr.comp(CompOp.Gte, NumExpr.stakeOf(TEST_USER_ADDRESS), NumExpr.literal(0))
      )
    );
    
    try {
      const nestedResult = await client.constraint.addTest.mutate({ constraint: nestedConstraint });
    ...
    try {
      ...
      const hasProductionNodes = networkState.visualization.includes('PRODUCTION NODES');
      console.log('   - Has Alpha Nodes:', hasAlphaNodes ? '✅' : '❌');
      console.log('   - Has Production Nodes:', hasProductionNodes ? '✅' : '❌');
    ...
    try {
      ...
      components.alphaNodes.forEach(alpha => {
        console.log(`      - ${alpha.key}: ${alpha.factCount} facts, ${alpha.successorCount} successors`);
        alpha.facts.forEach((fact, i) => {
          console.log(`        📄 Fact ${i + 1}: ${fact.type}`);
          console.log(`           Details: ${superjson.stringify(fact.details)}`);
        });
      });
      ...
      components.betaNodes.forEach(beta => {
        console.log(`      - Beta #${beta.index}: ${beta.tokenCount} tokens (${beta.uniqueCombinations} unique)`);
        beta.allTokens.forEach(token => {
          console.log(`        🎫 Token ${token.tokenIndex}: ${token.factCount} facts`);
      ...
      components.productionNodes.forEach(prod => {
        console.log(`      - ${prod.id}: ${prod.status} (${prod.activationCount} activations)`);
        console.log(`        📝 Constraint: ${prod.constraintId}`);
        console.log(`        🏗️  Structure: ${superjson.stringify(prod.constraint.bodyStructure)}`);
        
        prod.allActivations.forEach(activation => {
          console.log(`        ⚡ Activation ${activation.activationIndex}: ${activation.factCount} facts`);
      ...
      Object.entries(components.workingMemory.accountFacts).forEach(([account, facts]) => {
        console.log(`        👤 ${account}:`);
        facts.forEach(fact => {
          console.log(`           📄 ${fact.type}: ${superjson.stringify(fact.details, null, 8)}`);
        });
      });
      ...
      if (components.workingMemory.currentBlock) {
        console.log(`      - Current Block: #${components.workingMemory.currentBlock.number} (timestamp: ${components.workingMemory.currentBlock.timestamp})`);
      }
    ...
    console.log('🚨 Always-Fail Constraint (stake < 0): Should be violated + trigger callback');
    ...
    console.log('✅ Structured Network Components: Detailed inspection available');
    console.log();
    ...
    console.log('3. Simple comparisons now evaluate correctly (not always true)');
    ...
    console.log();
    console.log('🚀 FULL BOOLEAN EXPRESSION & VIOLATION CALLBACK TESTING COMPLETE!');
    console.log('==============================================================');
...
console.log('2. Start the torus-allocator app: npm run dev in apps/torus-allocator');
console.log('3. Keep the server console visible to see violation callback messages');
...
console.log('- Server console should show "🚨 CONSTRAINT VIOLATION DETECTED" messages');
