...
export async function GET(
  ...
  try {
    ...
    // If the icon is a string URL
    if (typeof agentMetadata.images.icon === 'string') {
      // If it's already an HTTPS url, redirect to it
      if (agentMetadata.images.icon.startsWith('https://')) {
        ...
        (
          ...
          >
            ...
            {agentMetadata.metadata.short_description && (
              <p style={{ fontSize: '24px', margin: '0', maxWidth: '800px', opacity: 0.8 }}>
...
async function generateDefaultOgImage(name: string) {
  ...
  return new ImageResponse(
    (
      ...
      >
        <svg width="150" height="150" viewBox="0 0 200 200" fill="none" style={{ marginBottom: '30px' }}>
        ...
        <h1 style={{ fontSize: '60px' }}>{name.length > 20 ? `${name.substring(0, 20)}...` : name}</h1>
