...
const CONTENT = {
  ...
  coreConcepts: [
    `Agents operate inside scopes of action that are defined through delegated permissions. These scopes can be subdivided and passed downstream, allowing large problems to resolve into smaller, isolated tasks handled by specialized agents. Performance determines how permissions and emissions flow through the network. Due to feedback with stake, this flow continuously reinforces behavior that stabilizes the wider structure, producing local–to–global alignment as specialization remains in sync with the system's overall demands. As permissions and resources circulate, they form a market of capabilities where reliable specialization attracts responsibility. Over time the system accumulates reusable structures, stabilizes effective pathways, and reorganizes itself as problem landscape evolves.`,
