...
import type { BooleanNodeData, PermissionNode, NodeCreationResult } from "./permission-node-types";
import { createChildNodeId, createEdgeId } from "./permission-node-types";

export function PermissionNodeBoolean({ id, data }: NodeProps<BooleanNodeData>) {
  ...
  const createChildNodes = useCallback((expression: BoolExpr): NodeCreationResult => {
  ...
  const handleTypeChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    switch (type) {
      case "Not":
        newExpression = BoolExpr.not(BoolExpr.base({ $: "InactiveUnlessRedelegated" }));
        break;
      case "And":
    ...
    // Create child nodes
    const { nodes: childNodes, edges: childEdges } = createChildNodes(newExpression);
  ...
  const handleCompOpChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  useEffect(() => {
    const currentNodes = getNodes();
    const hasChildren = currentNodes.some(node => node.id.startsWith(`${id}-`));
    
    if (!hasChildren && data.expression.$ !== "Base" && data.expression.$ !== "CompExpr") {
      const { nodes: childNodes, edges: childEdges } = createChildNodes(data.expression);
      setNodes(nodes => nodes.concat(childNodes));
      setEdges(edges => edges.concat(childEdges));
    }
  }, []);
