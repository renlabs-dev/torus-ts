import type { CustomGraphNode, PermissionDetails } from "../../permission-graph-types";
import {
  Card,
  CardContent,
  CardDescription,
...
export function GraphSheetDetailsPermission({ 
  ...
  const isIndefinite = detailedPermission?.permissions.durationType === "indefinite";
  ...
  return (
    <div className="w-full space-y-6 py-8">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge 
              variant={permissionData.permissionType === "emission" ? "default" : "secondary"}
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <h4 className="font-medium mb-2 flex items-center gap-2">
                {isIndefinite ? <InfinityIcon className="w-4 h-4" /> : <Clock className="w-4 h-4" />}
                Duration
              </h4>
              <div className="flex items-center gap-2">
                {isIndefinite ? (
                  ...
                  <div className="flex items-center gap-2">
                    <Badge 
                      variant={remainingBlocks > 0 ? "secondary" : "destructive"}
                      className="flex items-center gap-1"
                    >
                      {remainingBlocks > 0 ? (
                        <CheckCircle className="w-3 h-3 text-green-500" />
          ...
          {detailedPermission?.permissions.createdAt && (
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              Created {new Date(detailedPermission.permissions.createdAt).toLocaleDateString()}
            </div>
          )}
        </CardContent>
      </Card>
      ...
      {permissionData.permissionType === "emission" && detailedPermission?.emission_distribution_targets && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-mono text-sm">
                    {smallAddress(detailedPermission.emission_distribution_targets.targetAccountId)}
                  </p>
                  <p className="text-xs text-muted-foreground">
                ...
                <Badge variant="secondary">
                  Weight: {detailedPermission.emission_distribution_targets.weight}
                </Badge>
              </div>
            </div>
          </CardContent>
      ...
      {permissionData.permissionType === "emission" && detailedPermission?.emission_stream_allocations && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Layers className="w-5 h-5" />
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {detailedPermission.emission_stream_allocations.percentage.toFixed(2)}%
                </p>
              </div>
            </div>
          </CardContent>
