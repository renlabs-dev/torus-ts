import { pgTable, unique, check, serial, varchar, text, timestamp, foreignKey, index, integer, boolean, bigint, numeric, real, pgMaterializedView, pgEnum } from "drizzle-orm/pg-core"
...
export const comment = pgTable("comment", {
	...
	index().using("btree", table.itemType.asc().nullsLast().op("int4_ops"), table.itemId.asc().nullsLast().op("enum_ops"), table.userKey.asc().nullsLast().op("enum_ops")),
]);
