...
export const agentApplicationVoteType = pgEnum("agent_application_vote_type", [
  "ACCEPT",
  "REFUSE",
  "REMOVE",
]);
...
export const cadreCandidate = pgTable(
  "cadre_candidate",
  {
  ...
  (table) => [
    ...
    check(
      "discord_id_check",
      sql`(length((discord_id)::text) >= 17) AND (length((discord_id)::text) <= 20)`,
    ),
  ],
);

export const cadreVote = pgTable(
  "cadre_vote",
  {
    id: serial().primaryKey().notNull(),
    userKey: varchar("user_key", { length: 256 }).notNull(),
  ...
  (table) => [
    ...
    unique("cadre_vote_user_key_applicant_key_unique").on(
      table.userKey,
      table.applicantKey,
    ),
  ],
...
export const agent = pgTable(
  ...
  (table) => [
    ...
    check(
      "percent_check",
      sql`(weight_factor >= 0) AND (weight_factor <= 100)`,
    ),
  ],
);
...
export const commentInteraction = pgTable(
  "comment_interaction",
  {
    id: serial().primaryKey().notNull(),
    userKey: varchar("user_key", { length: 256 }).notNull(),
  ...
  (table) => [
    ...
    unique("comment_interaction_user_key_comment_id_unique").on(
      table.userKey,
      table.commentId,
    ),
  ],
...
export const computedAgentWeight = pgTable(
  "computed_agent_weight",
  {
    id: serial().primaryKey().notNull(),
    atBlock: integer("at_block").notNull(),
...
export const agentApplicationVote = pgTable(
  ...
  (table) => [
    ...
    unique("agent_application_vote_application_id_user_key_unique").on(
      table.applicationId,
      table.userKey,
    ),
  ],
...
export const penalizeAgentVotes = pgTable(
  ...
  (table) => [
    ...
    unique("penalize_agent_votes_agent_key_cadre_key_unique").on(
      table.agentKey,
      table.cadreKey,
    ),
    check(
      "percent_check",
      sql`(penalty_factor >= 0) AND (penalty_factor <= 100)`,
    ),
  ],
);

export const userAgentWeight = pgTable(
  "user_agent_weight",
  {
    id: serial().primaryKey().notNull(),
    userKey: varchar("user_key", { length: 256 }).notNull(),
  ...
  (table) => [
    ...
    unique("user_agent_weight_user_key_agent_key_unique").on(
      table.userKey,
      table.agentKey,
    ),
  ],
);
export const commentDigest = pgMaterializedView("comment_digest", {
  id: integer(),
  itemType: governanceItemType("item_type"),
