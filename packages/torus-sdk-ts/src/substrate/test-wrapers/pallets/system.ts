...
export const systemStorages = {
  ...
  lastRuntimeUpgrade: createStorageValue(
    "system",
    "lastRuntimeUpgrade",
    LAST_RUNTIME_UPGRADE_SCHEMA,
  ), // Last runtime upgrade as hex string
  blockWeight: createStorageValue(
    "system",
    "blockWeight",
    FRAME_SUPPORT_DISPATCH_PER_DISPATCH_CLASS_WEIGHT_SCHEMA,
  ), // Current block weight as hex string
  palletVersion: createStorageValue("system", "palletVersion", sb_number_int), // System pallet version
  upgradedToTripleRefCount: createStorageValue(
    "system",
    "upgradedToTripleRefCount",
    sb_bool,
  ), // Upgrade flag
  upgradedToU32RefCount: createStorageValue(
    "system",
  ...
  allExtrinsicsLen: createStorageValue(
    "system",
    "allExtrinsicsLen",
    sb_optional_number,
  ), // Total extrinsics length
  authorizedUpgrade: createStorageValue(
    "system",
    "authorizedUpgrade",
    sb_optional_string,
  ), // Authorized upgrade
  executionPhase: createStorageValue(
    "system",
    "executionPhase",
    sb_optional_string,
  ), // Current execution phase
  extrinsicCount: createStorageValue(
    "system",
    "extrinsicCount",
    sb_optional_number,
  ...
  // Map storages
  account: createStorageMap(
    "system",
    "account",
    sb_address,
    FRAME_SYSTEM_ACCOUNT_INFO_SCHEMA,
  ), // AccountId -> AccountInfo as hex string
  blockHash: createStorageMap("system", "blockHash", sb_blocks, sb_h256), // BlockNumber -> Hash
  eventTopics: createStorageMap(
    "system",
    "eventTopics",
    sb_h256,
    sb_to_primitive,
  ), // Hash -> Vec<EventIndex>
  extrinsicData: createStorageMap(
    "system",
    "extrinsicData",
    sb_number_int,
    sb_to_primitive,
  ), // ExtrinsicIndex -> Vec<u8>
} as const;
