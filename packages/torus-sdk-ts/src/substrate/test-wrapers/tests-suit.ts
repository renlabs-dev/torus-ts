...
export async function storageUnitTests(api: ApiPromise) {
  ...
  const torus0_values_get = {
    ...
    dividendsParticipationWeight:
      await storage.torus0.dividendsParticipationWeight.get(),
    ...
    maxRegistrationsPerBlock:
      await storage.torus0.maxRegistrationsPerBlock.get(),
    ...
    registrationsThisInterval:
      await storage.torus0.registrationsThisInterval.get(),
  ...
  console.log(
    `  • Agent update cooldown: ${torus0_values_get.agentUpdateCooldown}`,
  );
  console.log(
    `  • Max allowed validators: ${torus0_values_get.maxAllowedValidators}`,
  );
  ...
  const torus0_subscriptions = [
    storage.torus0.totalStake.subscribe((value) =>
      console.log("📡 TotalStake updated:", value),
    ),
    storage.torus0.burn.subscribe((value) =>
      console.log("📡 Burn updated:", value),
    ),
  ];
  ...
  const system_values_get = {
    ...
    upgradedToTripleRefCount:
      await storage.system.upgradedToTripleRefCount.get(),
  ...
  const system_subscriptions = [
    storage.system.number.subscribe((value) =>
      console.log("📡 BlockNumber updated:", value),
    ),
    storage.system.eventCount.subscribe((value) =>
      console.log("📡 EventCount updated:", value),
    ),
  ];
  ...
  const balances_subscriptions = [
    storage.balances.totalIssuance.subscribe((value) =>
      console.log("📡 TotalIssuance updated:", value),
    ),
  ];
  ...
  console.log(
    `  • DAO treasury address: ${governance_values_get.daoTreasuryAddress}`,
  );
  console.log(
    `  • Treasury emission fee: ${governance_values_get.treasuryEmissionFee}`,
  );
  ...
  console.log(
    `✅ Keys retrieved - Agents: ${torus0_map_keys.agents.length}, NamespaceCount: ${torus0_map_keys.namespaceCount.length}, Namespaces: ${torus0_map_keys.namespaces.length}`,
  );
  ...
  if (typeof rawNamespaceKey === "string") {
    ...
    if (primitiveResult.success) {
      ...
      if (
        rawNamespaceKey &&
        typeof rawNamespaceKey === "object" &&
        "account" in rawNamespaceKey
      ) {
  ...
  if (firstAgentKey) {
    ...
    const [agentError, _firstAgent_get] = await tryAsync(
      storage.torus0.agents.get(firstAgentKey as unknown as GenericAccountId),
    );
  ...
  if (firstNamespaceKey) {
    ...
    const [enumError, enumResult] = await tryAsync(
      storage.torus0.namespaceCount.get(accountEnum),
    );
    ...
    if (enumError === undefined) {
      ...
      const [objectError, objectResult] = await tryAsync(
        storage.torus0.namespaceCount.get(rawNamespaceKey),
      );
      ...
      if (objectError === undefined) {
        ...
        console.log(
          "⚠️ NamespaceCount get failed:",
          enumError.message.split("\n")[0],
        );
        allErrors.push({
          type: "Storage Query Error",
  ...
  if (torus0_map_keys.agents.length >= 3) {
    ...
    const agentKeys = torus0_map_keys.agents
      .slice(0, 3)
      .map((key) => new GenericAccountId(api.registry, key as AnyU8a));

    const [multiError, multipleAgents] = await tryAsync(
      storage.torus0.agents.multi(agentKeys),
    );

    if (multiError === undefined) {
      console.log(`✅ Multi query completed for 3 agents`);
      console.log(
        `📊 Multi agents result: ${multipleAgents.length} agents retrieved`,
      );
  ...
  console.log(
    `✅ Keys retrieved - Account: ${system_map_keys.account.length}, BlockHash: ${system_map_keys.blockHash.length}, EventTopics: ${system_map_keys.eventTopics.length}, ExtrinsicData: ${system_map_keys.extrinsicData.length}`,
  );
  ...
  if (firstSystemAccount) {
    console.log("\n--- Individual map .get() tests ---");
    const firstAccount_get = await storage.system.account.get(
      firstSystemAccount as unknown as GenericAccountId,
    );
  ...
  if (firstBlockNumber) {
    const firstBlock_get = await storage.system.blockHash.get(
      firstBlockNumber as unknown as AbstractInt,
    );
    console.log("✅ System blockHash get method completed");
    console.log(
      `📊 Block hash for block ${firstBlockNumber}: ${firstBlock_get}`,
    );
  }
  ...
  if (system_map_keys.account.length >= 2) {
    ...
    console.log(
      `📊 Multi system accounts result: ${multipleAccounts.length} accounts retrieved`,
    );
  ...
  console.log(
    `✅ Keys retrieved - Account: ${balances_map_keys.account.length}, Locks: ${balances_map_keys.locks.length}, Reserves: ${balances_map_keys.reserves.length}, Freezes: ${balances_map_keys.freezes.length}, Holds: ${balances_map_keys.holds.length}`,
  );
  ...
  const [agentApplicationsError, agentApplicationsKeys] = await tryAsync(
    storage.governance.agentApplications.keys(),
  );
  ...
  const [allocatorsError, allocatorsKeys] = await tryAsync(
    storage.governance.allocators.keys(),
  );
  if (allocatorsError) {
  ...
  const [proposalsError, proposalsKeys] = await tryAsync(
    storage.governance.proposals.keys(),
  );
  if (proposalsError) {
  ...
  const [whitelistError, whitelistKeys] = await tryAsync(
    storage.governance.whitelist.keys(),
  );
  if (whitelistError) {
  ...
  console.log(
    `✅ Keys retrieved - AgentApplications: ${governance_map_keys.agentApplications.length}, Allocators: ${governance_map_keys.allocators.length}, Proposals: ${governance_map_keys.proposals.length}, Whitelist: ${governance_map_keys.whitelist.length}`,
  );
  ...
  if (governance_map_keys.proposals.length > 0) {
    ...
    const [proposalGetError, proposal_get] = await tryAsync(
      storage.governance.proposals.get(firstProposal),
    );

    if (proposalGetError) {
      const errorMsg = proposalGetError.message;
      console.log(
        "⚠️ Governance proposal queries failed (schema issue):",
        errorMsg.split("\n")[0],
      );
    } else {
  ...
  console.log(
    `• Storage Values: 38+ entries (torus0: 18, system: 15, balances: 3, governance: 2+)`,
  );
  console.log(
    `• Storage Maps: 16+ entries (torus0: 3, system: 4, balances: 5, governance: 4+)`,
  );
  ...
  console.log(
    "🎯 Tested: get, display, keys, multi, subscribe across all pallets and storage types",
  );
  ...
  return {
    cleanup: async () => {
      ...
      await Promise.all([
        ...torus0_subscriptions,
        ...system_subscriptions,
        ...balances_subscriptions,
      ]);
    },
  };
}
...
export async function exampleUsage(api: ApiPromise) {
  ...
  if (agentKeys.length > 0) {
    ...
    const firstAgent = await storage.torus0.agents.get(
      firstAgentKey as unknown as GenericAccountId,
    );
    ...
    const multipleAgents = await storage.torus0.agents.multi(
      agentKeys.slice(0, 3).map((key) => key as unknown as GenericAccountId),
    );
  ...
  if (agentKeys.length > 0 && agentKeys[0]) {
    const accountData = await storage.balances.account.get(
      agentKeys[0] as unknown as GenericAccountId,
    );
  ...
  console.log(
    "💡 Subscriptions are active - they will show updates as the chain progresses",
  );
  return {
    cleanup: async () => {
